...
def _():
    ...
    from processes.vecoli import VEcoliProcess

    fp = Path("../ecoli_configs/single_cell.json")
    assert fp.exists()
    return Composite, Path, ProcessTypes, VEcoliProcess, fp
...
def _(Path):
    ...
    def main(duration: float | None = None, consolidate_metadata: bool = True):
        config_path = Path(CONFIG_DIR_PATH) / "dfba_steps.json"
        store_path = Path(OUT_DIR) / "dfba_steps.zip"
...
def _(Path):
    ...
    class DatasetZarr:
        ...
        def __init__(self, store_path: Path):
            self.root = self._open(store_path)
        
            tree_string = repr(self.structure)
            outdir = store_path.parent
            with open(f"{outdir}/zarr_tree.txt", "w", encoding="utf-8") as f:
                f.write(tree_string)
        ...
        def _open(self, store_path: Path) -> zarr.Group:
            self.store = ZipStore(f"{store_path!s}", mode="r")  # mode="r" ensures read-only
            return zarr.open_group(self.store, mode="r")

        def get(self, *path_parts):
            """
            :param *path_parts: (str) The hierarchical path to the dataset.
            ...
            for part in path_parts:
                node = node[part]  # traverse the hierarchy
            return node[:]  # return the array data


    def test_dataset_zarr():
        storepath = Path("artifacts/dfba_steps.zip")
        ds = DatasetZarr(storepath)
        cell_mass = ds.get("trajectory", "gauges", "cell_mass")
        assert cell_mass.shape == (1112,)
        assert (storepath.parent / "zarr_tree.txt").exists()
